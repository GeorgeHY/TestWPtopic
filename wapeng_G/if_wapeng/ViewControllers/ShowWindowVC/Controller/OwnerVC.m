...
-(void)teacherUserHeaderWithRequestWithID:(NSString *)teachID
{
    ...
    [http thirdRequestWithUrl:dUrl_DIA_1_6_1 succeed:^(NSObject *obj, BOOL isFinished) {
        ...
        ;
        MyAgentShowWindowView * view = (MyAgentShowWindowView *)[weakSelf.view viewWithTag:DTAG_TABLEHEADER];
...
-(void)startWindowTitleRequestWithUserID:(NSString *)userID
{
    ...
    [http thirdRequestWithUrl:dUrl_DIA_1_6_1 succeed:^(NSObject *obj, BOOL isFinished) {
        ...
        _item_userInfo.birthday = [arr[0] objectForKey:@"birthday"];
        
        _item_userInfo.gender = [[arr[0] objectForKey:@"gender"]intValue];
        
//        _item_userInfo.viewAttentionStatus =
        WindowHeaderView * view = (WindowHeaderView *)[weakSelf.view viewWithTag:DTAG_TABLEHEADER];
        ...
        view.ageLbl.text = [TimeTool getBabyAgeWithBirthday:_item_userInfo.birthday publicTime:now];
...
//                [temp addObjectsFromArray:[weakSelf.momentDict objectForKey:momentID]];
        ...
        if (isNilNull([weakSelf.momentDict objectForKey:momentID])) {
...
-(void)headerRereshing
{
    ...
    [self mHttpRequestWithUrl:dUrl_DIA_1_1_2 postDict:dict page:pageIndex];
}//必须被重写
...
#pragma mark -- 在橱窗全部的基础上附加的， 下拉刷新和上啦加载更多会被调用
-(void)addingHttpRequest
{
    
    for (Item_MyWindow * item in self.dataArray) {
        
        if (item.isOpen == YES) {
            
            [self newHttpRequestWithUrl:dUrl_DIA_1_3_2 momentID:item.momentID];
            
            [self newReplayRequestWithUrl:dUrl_DIA_1_4_2 momentID:item.momentID PageIndex:1];
            
        }
    }
  
}
#pragma mark -- 附加方法1， 在橱窗全部的基础上，就是为了默认展开
-(void)newReplayRequestWithUrl:(NSString *)url momentID:(NSString *)momentID PageIndex:(int)mPageIndex
{
    NSUserDefaults * d = [NSUserDefaults standardUserDefaults];
    ...
    [http fiveReuqestUrl:url postDict:postDict succeed:^(NSObject *obj, BOOL isFinished) {
        ...
        [SVProgressHUD showSuccessWithStatus:@"瞬间回复列表请求成功"];
          ...
          NSLog(@"count = %d zanCOunt = %d", self.momentDict.count, self.zanDict.count);
        ...
        if (self.momentDict.count == self.dataArray.count && self.zanDict.count == self.dataArray.count) {
...
#pragma mark -- 附加方法2， 在橱窗全部的基础上，就是为了默认展开
-(void)newHttpRequestWithUrl:(NSString *)url momentID:(NSString *)momentID
{
    NSUserDefaults * d = [NSUserDefaults standardUserDefaults];
...
- (void)viewDidLoad
{
    ...
    switch (self.userType) {
        case AGENT:
        {
            [self agentUserHeaderWithReqeustWithID:userInfoID];
        }
            break;
        case TEACHER:
        {
            [self teacherUserHeaderWithRequestWithID:userInfoID];
        }
            break;
        case PARENT:
        {
            [self startWindowTitleRequestWithUserID:userInfoID];
        }
            break;
        default:
            break;
...
-(WindowHeaderView *)myHeaderView
{
    WindowHeaderView * headerView = [WindowHeaderView instanceView];
...
-(MyAgentShowWindowView *)myAgentHeaderView
{
    MyAgentShowWindowView * headerView = [MyAgentShowWindowView instanceView];
    headerView.tag = DTAG_TABLEHEADER;
