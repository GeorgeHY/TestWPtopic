...
@interface PushAcitivityController ()<DoImagePickerControllerDelegate,UITextViewDelegate,faceViewDelegate>
{
...
@implementation PushAcitivityController
...
-(void)BuildUI{
    ...
    _validityView = [[UIView alloc]initWithFrame:CGRectMake(validity.frame.origin.x, CGRectGetMaxY(validity.frame), validity.frame.size.width, 15 * 4)];
...
-(void)didSelectPhotosFromDoImagePickerController:(DoImagePickerController *)picker result:(NSArray *)aSelected{
    ...
    __weak PushAcitivityController * weakSelf = self;
...
-(void)selectLabel{
//    [self.fastTextView resignFirstResponder];
//    SelectLabelViewController *selectLabel = [[SelectLabelViewController alloc]init];
...
//        return;
//    }
//    if (_contentTextStr.length == 0 || _contentTextStr == nil) {
//        
...
//    }
    ...
    SelectHotLabelVC * selectVC = [[SelectHotLabelVC alloc]init];
    selectVC.type = self.type;
    selectVC.title = @"选择标签";
    selectVC.mTitle = _titleText.text;
    selectVC.content = _contentTextStr;
    selectVC.limitTime = self.limitTime;
    selectVC.imageIDArray = _imageIDArr;
