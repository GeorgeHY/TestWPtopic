...
-(id)initWithFrame:(CGRect)frame superView:(UIView *)superView{
    ...
    if (self) {
        ...
        //可以自适应高度的文本输入框
        textView = [[UIExpandingTextView alloc] initWithFrame:CGRectMake(40, 5, 210, 53)];
...
//        [sendButton setBackgroundImage:[UIImage imageNamed:@"send"] forState:UIControlStateNormal];
        ...
        if (scrollView == nil) {
            ...
            scrollView = [[FaceScrollVeiw alloc]initWithFrame:CGRectMake(0, superView.frame.size.height, superView.frame.size.width, keyboardHeight)];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(receiveNotice:) name:@"faceName" object:nil];
...
-(void)newWithSuperView:(UIView *)superView
{
    
}

-(void)oldWithSuperView:(UIView *)superView
{
//    if (scrollView==nil) {
...
-(CGFloat)width
{
    CGSize size = [textView.text boundingRectWithSize:CGSizeMake(210, 1000000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:18]} context:nil].size;
    return size.height;
}
-(NSInteger)numberOfLineIntextView
{
    CGFloat f =  textView.internalTextView.contentSize.height;
    f -=33;
    
    int num = (int)f / 17;//font == 14的时候 d == 17
    num++;//1行
    NSLog(@"行数是 %d", num);
    return num;
...
//    int line = [self numberOfLineIntextView];
//    
//    
//    if (line < 4) {
//        //计算textView的高度
//        CGFloat f =  textView.internalTextView.contentSize.height; // 33 50 67
//        NSLog(@"高度%f", f);
//        
//        CGRect frame = toolBar.frame;
//        frame.size.height = (line - 1) * 20 + toolBarHeight;
//        
//        toolBar.frame = frame;
//        
//        frame = textView.frame;
//        frame.size.height = (line - 1) * 100 + 53;
//        
//        textView.frame = frame;
//        
//    }
//    CGFloat change = frame.size.height - toolBarHeight;
//    frame.origin.y= (_mFrame.size.height - toolBarHeight) -(line - 1) * 20;
...
-(void)inputKeyboardWillShow:(NSNotification *)notification{
    ...
    [UIView animateWithDuration:animationTime animations:^{
        ...
        if (toolBar.frame.size.height>45) {
            ...
            toolBar.frame = CGRectMake(0, keyBoardFrame.origin.y-45,  self.theSuperView.bounds.size.width,toolBarHeight);
        }
    }];
    [faceButton setBackgroundImage:[UIImage imageNamed:@"face"] forState:UIControlStateNormal];
    keyboardIsShow=YES;
...
-(void)receiveNotice:(NSNotification *)notify
{
    ...
    /*把输入过的*/
    [_dataArray addObject:notify.object];
    
    if ([notify.object isEqualToString:@"回退"]) {
        
        NSString * temp = textView.text;
        
        temp = [temp removeLastChar];
    ...
    str = [str stringByAppendingString:notify.object];
    textView.text = str;
}
@end
