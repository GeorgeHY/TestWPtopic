...
- (IBAction)nextBtnClick:(id)sender {
    ...
    [userDefaults setObject:self.babyName.text forKey:BABYNICKNAME];
    userDefaults = [NSUserDefaults standardUserDefaults];
    [userDefaults setObject:self.babyBirthday.text forKey:BABYBIRTHDAY];
    userDefaults = [NSUserDefaults standardUserDefaults];
    NSLog(@"生日%@",_dataManager.childBrith);
    ...
    //    if (self.twins.selected) {
    //        [userDefaults setObject:@"0" forKey:TWINS];
    //
    //        _dataManager.isUnique = @"0";
    //    }else{
    //        [userDefaults setObject:@"1" forKey:TWINS];
    //
    //        _dataManager.isUnique = @"1";
    //    }
    
    
    [userDefaults setObject: self.hospital.text forKey:HOSPITAL];
    
    _dataManager.brithHospital = self.hospital.text;
    NSLog(@"xiaohai独子%@",_dataManager.isUnique);
...
-(void) optionTableViewDataType:(TableFlagEnum) flag didSelectIndexPath:(NSIndexPath *)indexPath{
    switch (flag) {
        case EHospital://医院
        {
            ...
            self.search_Name=@"";
            [userDefaults setObject:item._id forKey:@"hospitalId"];
            [poplistview dismiss];
            tableFlag = EHospital;
            break;
        }
