...
{
    NSMutableDictionary *_selectImageNo;
}
@end
...
{
    [super viewDidLoad];
    _selectImageNo = [NSMutableDictionary dictionary];
    ...
    _tvAlbumList.frame = CGRectMake(0, _vBottomMenu.frame.origin.y + 20, self.view.frame.size.width, self.view.frame.size.height);
...
- (void)readAlbumList
{
    [ASSETHELPER getGroupList:^(NSArray *aGroups) {
        ...
        _tvAlbumList.frame = CGRectMake(_tvAlbumList.frame.origin.x, _tvAlbumList.frame.origin.y + 20, _tvAlbumList.frame.size.width, MIN(aGroups.count * 50, 200));
    }];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *str = _selectImageNo[@(indexPath.row)];
	if (_dSelected[@(indexPath.row)] == nil)
		[cell setSelectMode:NO imageName:str];
    else
		[cell setSelectMode:YES imageName:str];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (_nMaxCount > 1 || _nMaxCount == DO_NO_LIMIT_SELECT)
    {
		...
		if ((_dSelected[@(indexPath.row)] == nil) && (_nMaxCount > _dSelected.count))
		{
            ...
            NSString *str = [NSString stringWithFormat:@"image%lu.png",(unsigned long)_dSelected.count];
            [_selectImageNo setObject:str forKey:@(indexPath.row)];
//            cell.selectNo.image = [UIImage imageNamed:str];
			[cell setSelectMode:YES imageName:str];
		}
		else
		{
			// unselect
            cell.selectNo.image = nil;
            [_selectImageNo removeObjectForKey:@(indexPath.row)];
			[_dSelected removeObjectForKey:@(indexPath.row)];
			[cell setSelectMode:NO imageName:nil];
		}
